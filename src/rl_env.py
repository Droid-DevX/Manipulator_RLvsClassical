...
MODEL_PATH = os.path.join("mujoco_menagerie", "franka_emika_panda", "pick_scene_torque.xml")
...
class PandaPickPlaceEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # ================================================================
        # Keep a persistent target so PPO produces a trajectory rather than
        # commanding a fresh position from the instantaneous robot state.
        ...
        for substep in range(CONTROL_SUBSTEPS):
            ...
            # qfrc_bias compensates the robot's bias forces (gravity and
            # velocity-dependent terms), matching the standalone PID pipeline.
            ...
            # When an external viewer is attached (watch_policy.py), update
            # it during the physics rollout rather than only once per PPO
            # action. This makes motion visually smooth without changing
            # the control or physics timestep.
            if (
